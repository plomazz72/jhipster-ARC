...
import com.bbpm.it.config.KafkaProperties;
...
class JhipsterArc01KafkaResourceIT {
    ...
    void setup() {
        ...
        JhipsterArc01KafkaResource kafkaResource = new JhipsterArc01KafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/jhipster-arc-01-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jhipster-arc-01-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
